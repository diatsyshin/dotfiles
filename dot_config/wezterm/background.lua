...
local BG_IMAGES = {
	{ File = wezterm.home_dir .. "\\.config\\wezterm\\backgrounds\\weztermbackground.png" },
	{ File = wezterm.home_dir .. "\\.config\\wezterm\\backgrounds\\wezterm_christmasbackground.png" },
	false, -- No image option (using false instead of nil)
}

local BG_IMAGES_COUNT = 3 -- Explicitly define the count

local OPACITY_OPTIONS = { 0.8, 0.9, 1.0 }
...
local function getNextItem(table, currentIndex)
	...
	wezterm.log_info("Bgimage selected: " .. nextIndex)
